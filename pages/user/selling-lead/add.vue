...
export default {
  ...
  methods: {
      ...
      await this.$axios.$post('/api/find_category', {id: val})
        .then(response => {
          ...
          if (response.child_ids && response.child_ids.length) {
            ...
            this.sellItems.all_related_category.push(val);
    ...
    submitProduct() {
      ...
      formData.append("video", this.sellItems.video);


      //Multiple files
      var ins = this.sellItems.images.length;
      for (var x = 0; x < ins; x++) {
        formData.append("images[]", this.sellItems.images[x]);
      }
    ...
    updateAttrFromCustom() {//Update attribute from custom user attribute
      this.sellItems.attribute = this.mainAttr;
      for (var i = 0; i < this.dynamicAttr.length; i++) {
        this.sellItems.attribute[this.dynamicAttr[i].label] = this.dynamicAttr[i].value;
      }
    },
    goBack() {
