<template>
  <div class="pa-8 secondary-background company-details-tabs-content about-company">
    <v-row class="mb-8">
      ...
      <v-col cols="12" lg="4" class="d-lg-flex justify-lg-end">
        <v-carousel :show-arrows="false" class="company-image">
          <v-carousel-item v-for="(item, i) in companyInfo.images" :key="i">
            <nuxt-img format="webp" :src="getFile(item, 'image')" :alt="companyInfo.company_name" />
          </v-carousel-item>
        </v-carousel>
      </v-col>
    </v-row>
    ...
    <v-row v-if="companyInfo.roles && companyInfo.roles.includes('logistics')">
      <v-col cols="12" class="mb-8">
        <h2 class="title">
          <v-icon class="secondary-color">mdi-draw</v-icon>
          Request
        </h2>
        <v-col cols="12" class="mb-8">
          <LogisticsRequest :companyInfo="companyInfo"/>
        </v-col>
      </v-col>
    </v-row>
    <v-row v-else>
...
import LogisticsRequest from "../../company-details/logistics-request"
